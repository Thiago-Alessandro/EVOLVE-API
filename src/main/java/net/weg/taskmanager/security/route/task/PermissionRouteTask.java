...
public class PermissionRouteTask implements AuthorizationManager<RequestAuthorizationContext> {
        ...
        if (statusId == null) {
            if (userId == null) {
                if (existsByIdAndAssociatesContaining(taskId, user)) {
                    return new AuthorizationDecision(isUserAuthorized(project.getId(), user, Permission.valueOf(request.getMethod())));
                }
            }
    ...
    private boolean isUserAuthorized(Long projectId, User user, Permission auth) {
        return user.getProjectsAcess()
                .stream().filter(projectAcess -> projectAcess.getProjectId().equals(projectId))
                .anyMatch(projectAcess -> projectAcess.getAcessProfile().getAuths().contains(Permission.EDIT_PROJECT_INFO)
                );
    }
}
